...
int main(void)
{
  /* USER CODE BEGIN 1 */
	...
	int n_sinus = 0;
	n_sinus = sizeof(sinus_lut)/sizeof(uint32_t);
	// I don't take the last element because it is equal to the first
	n_sinus -= 1;
	// My square lookup table
	uint32_t square_lut[]={
			0,4095
	};
	// Calculate the number of elements in my square lookup table
	int n_square = 0;
	n_square = sizeof(square_lut)/sizeof(uint32_t);
  /* USER CODE END 1 */
  ...
  /* USER CODE BEGIN 2 */
  // Start DAC with DMA
  //HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint32_t*)sinus_lut, (uint32_t)n_sinus, DAC_ALIGN_12B_R);
  HAL_DAC_Start_DMA(&hdac1, DAC_CHANNEL_1, (uint32_t*)square_lut, (uint32_t)n_square, DAC_ALIGN_12B_R);
